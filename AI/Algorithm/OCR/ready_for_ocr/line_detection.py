...
def detect_and_crop_by_lines(image_path, output_dir, is_answer):
    # 이미지 읽기
    ...
    for i in range(len(y_coordinates) - 1):
        ...
        # 크롭된 이미지 저장
        if(is_answer):
            output_path = os.path.join(output_dir, f'answer_{i+1}_{y_start}_{y_end}.jpg')
        else:    
            output_path = os.path.join(output_dir, f'question_{i+1}_{y_start}_{y_end}.jpg')
            # 여기까지 파일명에 크롭된 이미지의 원래 y_top, y_bottom 작성 완료 ㅋㅋ by 다훈 0411
        cv2.imwrite(output_path, cropped)
        print(f"Saved cropped image to {output_path}")
    
    print(f"Total number of images created: {len(os.listdir(output_dir))}")
...
def main():
    ...
    detect_and_crop_by_lines(image_path, output_dir, is_answer)
