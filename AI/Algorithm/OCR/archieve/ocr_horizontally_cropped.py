'''
이 스크립트는 EasyOCR을 사용하여 가로로 잘린 이미지에서 텍스트를 인식하고, 인식된 결과를 텍스트 파일로 저장하며, 시각화된 이미지를 생성하여 저장합니다. 영어와 한국어를 인식할 수 있습니다.
'''
...
# 이미지 경로 설정
base_dir = '/home/jdh251425/2025_DKU_Capstone/AI/Algorithm/OCR/cropped_datasets/horizontally_cropped'
...
def process_images(input_dir, output_dir):
    for filename in os.listdir(input_dir):
        if filename.endswith(('.jpeg', '.jpg', '.png')):
            ...
            # 시각화 이미지 저장 경로
            vis_image_path = os.path.join(output_dir, f'{os.path.splitext(filename)[0]}_vis.jpg')
